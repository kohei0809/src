...
// 計画停止前にコミュニケーションを行う
public class TargetPathAgent_PlannedStopping implements IAgent{
    ...
    public TargetPathAgent_PlannedStopping(int id, LitterSpawnPattern pattern, GridGraph graph, List<Integer> excludeNodes){
        setRobotID(id);
        ...
        if(action == AgentActions.move){
            if(position == target){
                if(isChargeRequired || targetter.getIsChargeRequired()){
                    ...
                    calculateSearchNode(data.getTime());
                    calculateCenterNode();
                }
            }
        }
    ...
    private void calculateSearchNode(int time){
        searchNodes.clear();
        ...
        if(preStop){
            if(searchNodeStartIndex > 0 && searchNodeStartIndex != searchNodeNumber){
                ...
                System.out.println(robotID + ":searchNodeStartIndex > 0");
            }
        }
