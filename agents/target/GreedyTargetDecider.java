...
public class GreedyTargetDecider implements ITargetDecider{
    ...
    public GreedyTargetDecider(int id, IGraph map, LitterSpawnPattern pattern, boolean isA, int seed, List<Integer> excludeNodes){
        robotID = id;
    ...
    public void update(TargetPathAgentStatus status){
        ...
        //Find new target
        if(data.getPosition() == status.getTarget()){
            ...
            for(int node : nodes){
                ...
                if(node != data.getPosition()){
                    array.add(i, new AbstractMap.SimpleEntry<Integer, Double>(node, exp * discountExpectation(status.getObservedData(), node)));
                    sum += exp;
                }
                else{
