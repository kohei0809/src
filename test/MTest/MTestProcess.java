...
public class MTestProcess implements Runnable{
    ...
    public MTestProcess(LogManager2 logManager, CountDownLatch startLatch, int runs, AgentType agentType, String env, int t, String targetter,
    ...
    public void run(){
        try{
            ...
            logManager.setLogDirectory("log/MTest/" + agentType + "/" + env + "/" + t + "-" + targetter + "/" 
                    + date + " robots-number=" + robots + "_" + counter);
            MTestSimulationFactory factory = new MTestSimulationFactory();
            factory.setLocalNumbers(s, s+3, s+1, p, s+2, t, robots, scale, true, env, behaviorType, agentType, requirement, correction);

            MTestSimulation simulation = new MTestSimulation(factory);
            simulation.reset();
            simulation.run(runs);
