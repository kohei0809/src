...
public class OTestProcess implements Runnable{
    ...
    public OTestProcess(LogManager2 logManager, CountDownLatch startLatch, int runs, AgentType agentType, String env, int t, String targetter,
    ...
    public void run(){
        try{
            ...
            logManager.setLogDirectory("log/OTest/" + agentType + "/" + env + "/" + t + "-" + targetter + "/" 
                    + date + " robots-number=" + robots + "_" + counter);
            OTestSimulationFactory factory = new OTestSimulationFactory();
            factory.setLocalNumbers(s, s+3, s+1, p, s+2, t, robots, scale, true, env, behaviorType, agentType, requirement, correction);

            OTestSimulation simulation = new OTestSimulation(factory);
            simulation.reset();
            simulation.run(runs);
