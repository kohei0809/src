...
public class STestProcess implements Runnable{
    ...
    public STestProcess(LogManager2 logManager, CountDownLatch startLatch, int runs, AgentType agentType, String env, int t, String targetter,
    ...
    public void run(){
        try{
            ...
            logManager.setLogDirectory("log/STest/" + agentType + "/" + env + "/" + t + "-" + targetter + "/" 
                    + date + " robots-number=" + robots + "_" + counter);
            STestSimulationFactory factory = new STestSimulationFactory();
            factory.setLocalNumbers(s, s+1, p, s+2, t, robots, scale, true, env, behaviorType, agentType);

            STestSimulation simulation = new STestSimulation(factory);
            simulation.reset();
            simulation.run(runs);
