...
public class UTestProcess implements Runnable{
    ...
    public UTestProcess(LogManager2 logManager, CountDownLatch startLatch, int runs, AgentType agentType, String env, int t, String targetter,
    ...
    public void run(){
        try{
            ...
            logManager.setLogDirectory("log/UTest/" + agentType + "/" + env + "/" + t + "-" + targetter + "/" 
                    + date + " robots-number=" + robots + "_" + counter);
            UTestSimulationFactory factory = new UTestSimulationFactory();
            factory.setLocalNumbers(s, s+3, s+1, p, s+2, t, robots, scale, true, env, behaviorType, agentType, requirement, correction, s+4);

            UTestSimulation simulation = new UTestSimulation(factory);
            simulation.reset();
            simulation.run(runs);
