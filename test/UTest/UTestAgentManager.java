...
public class UTestAgentManager implements IAgentManager{
    ...
    LogWriter2 onebyoneLogger;
    LogWriter2 agentSuccessLogger;
    LogWriter2 stopNumberLogger;
    LogWriter2 restartLogger;
    LogWriter2 cycleLogger;
    LogWriter2 batteryLogger;
    

    public UTestAgentManager(IEnvironment environment, BehaviorType behavior, int seed, GridGraph graph){
        ...
        onebyoneLogger = LogManagerContext.getLogManager().createWriter2("onebyoneLogger");
        agentSuccessLogger = LogManagerContext.getLogManager().createWriter2("agentSuccess");
        stopNumberLogger = LogManagerContext.getLogManager().createWriter2("stopNumber");
        restartLogger = LogManagerContext.getLogManager().createWriter2("restart");
        cycleLogger = LogManagerContext.getLogManager().createWriter2("Cycle");
        //cycleLogger.writeLine("time,100,300,900\n");
        batteryLogger = LogManagerContext.getLogManager().createWriter2("Battery");
    }
    ...
    public void move(){
        ...
        printAgentCycle(3600);
        //printAgentPositionCount(1000000);
        printAgentRemainBattery(10000);
    }
    ...
    public void onebyoneStoppable(){
        ...
        if(isStop){
            if(time == (stop_time + scount*stop_interval)){
                ...
                for(Map.Entry<IAgent, Integer> pair : agents){
                    ...
                    //エージェントが停止していなければ
                    if(agentActions.get(id) != AgentActions.stop){
                    	int success = agent.getSuccessCount();
                    	int failure = agent.getFailureCount();
                    	agent.resetSuccessCount();
                    	agentSuccessLogger.write(time + "," + id + "," + success + "," + failure + ",");
                    	if(success == 0 && failure == 0){
                    		agentSuccessLogger.writeLine("");
                    		continue;
                    	}
                    	else{
                    	
                    		double value = (double)success / (success + failure);
                    		sum += value;
                            ...
                            count++; 
                            agentSuccessLogger.writeLine(Double.toString(value));
                            //pausingCount += (success + failure);
                            double K = agent.getCorrection();
                            if(max < K){
                            	max = K;
                            }
                    	}
    ...
    public void multipleStoppable(){
        ...
        if(isStop){
            if(time == (stop_time + scount*stop_interval)){         
                ...
                double prob = (double)sSum / pausingCount;
                onebyoneLogger.writeLine(time + "," + sSum + "," + fSum + "," + prob);
                
                if(((pausingCount >= stop_check) && (max >= 1.0)) || ((pausingCount >= 2*stop_check) && (max >= 0.9))){
                	System.out.println("Agents stop at " + stop_time + "ticks!");
                	double check = 0;
                    if(pausingCount >= 2*stop_check){
                    	check = 0.9;
                        stop_robots_number = (int)(Math.log(pausingCount / (2*stop_check)) / Math.log(2)) + 1;
                        stopNumberLogger.writeLine(time + "," + stop_robots_number + "," + pausingCount + "," + 2*stop_check);
                    }
                    else if(pausingCount >= stop_check){
                    	check = 1.0;
                        stop_robots_number = 1;
                        stopNumberLogger.writeLine(time + "," + stop_robots_number + "," + pausingCount + "," + stop_check);
                    }
                    ...
                    for(int i=0; i<stop_number; i++) {
                        double value = array.get(i).getValue();
                        int delete_id = array.get(i).getKey();
                        if(agents.get(delete_id).getKey().getCorrection() < check) {
                        	break;
                        }                       
                        if(agentActions.get(delete_id) != AgentActions.stop){
                        	agentActions.put(delete_id, AgentActions.stop);
  ...
  //降降順にエージェントの途中停止と再開
    ...
    //エージェントの巡回サイクルを記録
    private void printAgentCycle(int interval){
        int time = environment.getTime();
        int[] cycle = {0, 0, 0}; //100, 300, 900
        ...
        if(time % interval == 0){
            for(Map.Entry<IAgent, Integer> pair : agents){
                ...
                if(agentActions.get(agent.getRobotID()) != AgentActions.stop){
                    cycle[agent.getCycleIndex()]++;
                }
            }
            cycleLogger.writeLine(time + "," + cycle[0] + "," + cycle[1] + "," + cycle[2]);
        }
    }
    ...
    private void printAgentRemainBattery(int interval){
        int time = environment.getTime();
        RobotDataCollection robots = environment.getRobotDataCollection();
        ...
        if(time % interval == 0){
            for(Map.Entry<IAgent, Integer> pair : agents){
                ...
                int id = pair.getValue();
                if(agentActions.get(agent.getRobotID()) != AgentActions.stop){
                    batteryLogger.writeLine(time + "," + id + "," + robots.getRobotData(id).getBatteryLevel() + "," + agentActions.get(agent.getRobotID()));
                }
            }
        }
    }
