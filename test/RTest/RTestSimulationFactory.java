...
import agents.target.EpsilonGreedyTargetDecider;
import agents.target.GreedyTargetDecider;
import agents.target.IntervalGreedyTargetDecider;
...
public class RTestSimulationFactory extends SimulationFactory{
    //計画停止，コミュニケーションなし
    ...
    RTestAgentManager agentManager;
    //RTestAgentManager2 agentManager;
    Evaluator evaluator;
    GridGraph graph;
	...
	int basePosition, robotCount = 1, scale = 50;
    ...
    public void setLocalNumbers(int eseed, int pseed, int pnumber, int tseed, int tnumber, int robotC, int s, 
            boolean isA, String pattern, BehaviorType bType, AgentType aType){
        environmentSeed = eseed;
        patherSeed = pseed;
    ...
    public void make(){
        ...
        agentManager = new RTestAgentManager(environment, beType, seeds[environmentSeed + 3], graph);
        //agentManager = new RTestAgentManager2(environment, beType, seeds[environmentSeed + 3], graph);
    ...
    public void createLitterSpawnPattern(){
        ...
        for(int node : graph.getAllNode()){
            ...
            if(patternName == "Test") {
               ...
               //System.out.println("node=" + node + " prob=" + prob);
            }

            else if(patternName == "Test2") {
    ...
    protected void createAgent(){
        ...
        for(int robot : robots.keySet()){
            ...
            switch(targetterNumber){
                ...
                case 4:
                default:
                    AMTDSLearningTargetDecider decider6 = new AMTDSLearningTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt());
                    ...
                    break;
                case 5:
                    EpsilonGreedyTargetDecider decider = new EpsilonGreedyTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt());
                    decider.addTargetDecider(new RandomTargetDecider(robot, graph, tRand.nextInt(), excludeNodes));
                    decider.addTargetDecider(new GreedyTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt(), excludeNodes));
                    targetter = decider;
                    break;  
                case 6:
                    EpsilonGreedyTargetDecider decider2 = new EpsilonGreedyTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt());
                    decider2.addTargetDecider(new RandomTargetDecider(robot, graph, tRand.nextInt(), excludeNodes));
                    decider2.addTargetDecider(new MyopiaSweepTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt(), excludeNodes));
                    targetter = decider2;
                    break;
                case 7:
                    IntervalGreedyTargetDecider decider3 = new IntervalGreedyTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt());
                    decider3.addTargetDecider(new LongIntervalTargetDecider(robot, graph, tRand.nextInt(), excludeNodes));
                    decider3.addTargetDecider(new GreedyTargetDecider(robot, graph, spawnPattern, isAccumulate, tRand.nextInt(), excludeNodes));
                    targetter = decider3;
                    break;
            }
