...
public class TTestProcess implements Runnable{
    ...
    public TTestProcess(LogManager2 logManager, CountDownLatch startLatch, int runs, AgentType agentType, String env, int t, String targetter,
    ...
    public void run(){
        try{
            ...
            logManager.setLogDirectory("log/TTest/" + agentType + "/" + env + "/" + t + "-" + targetter + "/" 
                    + date + " robots-number=" + robots + "_" + counter);
            TTestSimulationFactory factory = new TTestSimulationFactory();
            factory.setLocalNumbers(s, s+3, s+1, p, s+2, t, robots, scale, true, env, behaviorType, agentType, requirement, correction);

            TTestSimulation simulation = new TTestSimulation(factory);
            simulation.reset();
            simulation.run(runs);
